...
@section('content')
    ...
    @include('plano_de_contas.filter')
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('planodecontas/create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('plano_de_contas.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $planoDeContas->currentPage() }} de {{ $planoDeContas->lastPage() }} página(s), total de {{ $planoDeContas->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'planodecontas/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = '/planodecontas/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Todo(s) o(s) Plano(s) de Conta(s) inativada(s) não poderá(ão) ficar disponivél no sistema!",
                    type: "warning",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar seus Plano(s) de contas selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus Plano(s) de contas não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
