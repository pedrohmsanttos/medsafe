...
@section('content')
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/negocioProdutos" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                <div class="col-md-10">
                    <h2>Lista de Negocio Produtos</h2>
                </div>
                <ul class="header-dropdown ">
                     <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('negocioProdutos.create') !!}">Adicionar novo</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('negocio_produtos.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $negocioProdutos->currentPage() }} de {{ $negocioProdutos->lastPage() }} página(s), total de {{ $negocioProdutos->total() }} registros <p id="selecionados"></p> </p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $negocioProdutos->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'negocio_produtos/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Todos o(s) Negócio(s) de Produto(s) inativados perderá(ão) o acesso ao sistema!",
                    type: "warning",
                    ...
                    if (isConfirm) {
                        var negocio_produtos = [];
                        $.each($("input:checked"), function(){            
                            if($(this).val() != 'on'){
                                negocio_produtos.push($(this).val());
                            }
                        });
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os negocio_produtos selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus negocio_produtos não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
