...
@section('content')
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('ocorrencias.create') !!}">Novo</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('ocorrencias.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $ocorrencias->currentPage() }} de {{ $ocorrencias->lastPage() }} página(s), total de {{ $ocorrencias->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'ocorrencias/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os ocorrencias selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus ocorrencias não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
