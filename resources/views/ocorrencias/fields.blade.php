...
<div class="col-md-6">
        <div class="form-group">
            ...
            <div class="form-line {{$errors->has('category_id') ? 'focused error' : '' }}">
                @if(!empty($clientes->user_id))
                    <select id="user_id" name="user_id" class="form-control show-tick" disabled>
                        <option disabled selected value="">Selecione o Cliente</option>
                        @foreach($clientes as $cliente)
                            <option value="{{$cliente->user_id}}" 'disabled' >{{$cliente->nomeFantasia}}</option>
                        @endforeach
                    </select>
                @else
                    <select id="user_id" name="user_id" class="form-control show-tick">
                        <option disabled selected value="">Selecione o Cliente</option>
                        @foreach($clientes as $cliente)
                            <option value="{{$cliente->user_id}}"  >{{$cliente->nomeFantasia}}</option>
                        @endforeach
                    </select>
                @endif
            </div>
...
<div class="col-md-6">
        <div class="form-group">
            ...
            <div class="form-line {{$errors->has('category_id') ? 'focused error' : '' }}">
                @if(!empty($categorias->category_id))
                    <select id="category_id" name="category_id" class="form-control show-tick" disabled>
                        ...
                        @foreach($categorias as $cat)
                            <option value="{{$cat->id}}"  'disabled' >{{$cat->descricao}}</option>
                        @endforeach
                    </select>
                @else
