...
@section('content')
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/tipoAtividades" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                <div class="col-md-10">
                    <h2>Lista de Tipo Atividades</h2>
                </div>
                <ul class="header-dropdown ">
                     <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('tipoAtividades.create') !!}">Adicionar novo</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('tipo_atividades.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $tipoAtividades->currentPage() }} de {{ $tipoAtividades->lastPage() }} página(s), total de {{ $tipoAtividades->total() }} registros <p id="selecionados"></p> </p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $tipoAtividades->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'tipoAtividades/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                //window.location = 'tipo_atividades/'+$('.checkbox:checked')[indice].value+'/edit'
                swal({
                    title: "Você tem certeza?",
                    text: "Toda(s) a(s) atividade(s) inativada(s) perderá(ão) o acesso ao sistema!",
                    type: "warning",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os tipo_atividades selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus tipo_atividades não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
