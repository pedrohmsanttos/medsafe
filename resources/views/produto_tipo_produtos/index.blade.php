...
@section('content')
    ...
    @include('produto_tipo_produtos.filter')
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('produtoTipoProdutos/create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('produto_tipo_produtos.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $produtoTipoProdutos->currentPage() }} de {{ $produtoTipoProdutos->lastPage() }} página(s), total de {{ $produtoTipoProdutos->total() }} registros<p id="selecionados"></p> </p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $produtoTipoProdutos->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'produtoTipoProdutos/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'produtoTipoProdutos/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Toda(s) a(s) Tabela(s) de Preço(s) inativada(s) não poderá(ão) ser mais utilizadas no sistema!",
                    type: "warning",
                    ...
                    if (isConfirm) {
                        var produtoTipoProdutos = [];
                        $.each($("input:checked"), function(){            
                            if($(this).val() != 'on'){
                                produtoTipoProdutos.push($(this).val());
                            }
                        });
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os preços selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Preços não foi(ram) inativado(s)", "error");
                    }
                });
            }else{
            ...
            if(urlBusca != '' && urlBusca.indexOf(opCampo) != -1 ){
                ...
                if(valor != '' &&
                    ...
                    if($("#filtro").val() == 'orderBy' && opCampo != 'situacao'){
                        if(urlOrdem == ''){
                            if ($("#campo").val()=='tipo'){
                                urlOrdem = '&orderBy=tipo_produto_id'+'&sortedBy='+valor;
                            }
                            else if (($("#campo").val()=='categoria')){
                                urlOrdem = '&orderBy=categoria_produto_id'+'&sortedBy='+valor;
                            }
                            else{
        ...
        var filtrar = function(){
            ...
            console.log(urlOrdem);
            if (urlFiltro == '&searchFields=produto_id:=' || urlFiltro == '&searchFields=tipo:=' || urlFiltro == '&searchFields=categoria:='){
                urlCampos = urlCampos + ":=";
                console.log(urlCampos);
                window.location = window.location.origin + window.location.pathname + urlCampos + urlOrdem + situacao + boolean;
            }
            
            else if (urlFiltro == '&searchFields=produto_id:like' || urlFiltro == '&searchFields=tipo:like' || urlFiltro == '&searchFields=categoria:like'){
                urlCampos = urlCampos + ":like";
                console.log(urlCampos);
                window.location = window.location.origin + window.location.pathname + urlCampos + urlOrdem + situacao + boolean;
            }
