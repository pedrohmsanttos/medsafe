...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select id="campo">
                                ...
                                <option value="produto_id">Produto/Serviço</option>
                        ...
                        <div class="col-md-3 form-line">
                            ...
                            <div id="valorProduto">
                                <select data-live-search="true" id="produto">
                            ...
                            <input type="text"  id="valor1" class="form-control dinheiro">
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/medsafeBeneficios" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
                                    ...
                                    @foreach($filters as $filter)
                                        <tr class="teste">
                                            ...
                                            @if(strlen($nomeProduto) >0 )
                                                <td class="center" id="tr-init">{{ $nomeProduto }}</td>
                                            @else
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                <div class="col-md-10">
                    <h2>Lista de Beneficios MEDSafer</h2>
                </div>
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('medsafeBeneficios.create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('medsafe_beneficios.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $medsafeBeneficios->currentPage() }} de {{ $medsafeBeneficios->lastPage() }} página(s), total de {{ $medsafeBeneficios->total() }} registros <p id="selecionados"></p> </p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $medsafeBeneficios->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'medsafeBeneficios/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'medsafeBeneficios/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                //window.location = 'medsafe_beneficios/'+$('.checkbox:checked')[indice].value+'/edit'
                swal({
                    ...
                    if (isConfirm) {
                        var medsafe_beneficios = [];
                        $.each($("input:checked"), function(){            
                            if($(this).val() != 'on'){
                                medsafe_beneficios.push($(this).val());
                            }
                        });
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os medsafe_beneficios selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus medsafe_beneficios não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
        ...
        $(".add").click(function(){
            ...
            if(opFiltro == 'orderBy'){
                ...
                valor = $('#status').val();
                situacao += valor;
            }else if(opCampo == 'produto_id'){
                valor = $('#produto').val();
            }else{
                if($('#campo').val() == 'valor'){
                    $('#valor').val($('#valor1').val());
                }
                valor = $('#valor').val();
            }
        ...
        $('#campo').on('change',function() {
            ...
            if(opcao == 'situacao'){
                ...
                $('#valorProduto').hide();
            }else if(opcao == 'produto_id' && opSelecionada != 'orderBy'){
                $('#valor').hide();
                $('#valorOrdem').hide();
                $('#valorSituacao').hide();
                $('#campo').selectpicker('render');
        ...
        $('#filtro').on('change',function() {
            ...
            var opcao = $(this).val();
            if(opcao == 'contem'){
                $("#campo option[value='valor']").prop('disabled',true);
            }
            if(opcao != 'contem'){
                $("#campo option[value='valor']").prop('disabled',false);
            }
            if(opcao == 'orderBy'){
                $('#valor').hide();
                $('#valorSituacao').hide();
                $('#valorOrdem').show();
                $("#campo option[value='situacao']").prop('disabled',true);
                $("#campo option[value='produto_id']").prop('disabled',true);
                $("#campo").val('');
                $('#campo').selectpicker('render');
                $('#valorProduto').hide();
            }else{
                ...
                $('#valorSituacao').hide();
                $("#campo option[value='situacao']").prop('disabled',false);
                $("#campo option[value='produto_id']").prop('disabled',false);
                $("#campo").val('');
                $('#campo').selectpicker('render');
            }
        });
