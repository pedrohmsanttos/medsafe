            ...
            <div class="card">
                <div class="header">
                    ...
                    <ul class="header-dropdown m-r-0">
                        <li>
                            <a href="{{ url('relatorio/negociosPeriodo') }}">
                                Voltar
                            </a>
                        </li>
                    </ul>
                            ...
                            <tbody>
                                @if(!empty($negocios))
                                    @foreach($negocios as $negocio)
                                        @php   $produtoServico = $tipoProduto = $categoriaProduto = '' @endphp
                                        @foreach($negocio->itens()->get() as  $key => $item)
                                            
                                            @if ( !empty($item)  )
                                                @php 
                                                    $produtoServico .= $item->tabelaPreco()->first()->produto()->first()->descricao . ","; 
                                                    $tipoProduto .= $item->tabelaPreco()->first()->tipoProduto()->first()->descricao . ","; 
                                                    $categoriaProduto .= $item->tabelaPreco()->first()->categoriaProduto()->first()->descricao . "," 
                                                @endphp
                                            @endif
                                        @endforeach
                                        ...
                                        <tr>
                                            <td>{{ $negocio->usuario }}</td>
                                            <!-- <td>{{ $negocio->titulo }}</td> -->
                                            <td>{{ removeLastCharacter( $produtoServico ) }}</td>
                                            <td>{{ removeLastCharacter( $tipoProduto ) }}</td>
                                            <td>{{ removeLastCharacter( $categoriaProduto ) }}</td>
                                            <td>R$ {{ number_format((float) $negocio->valor, 2, ',', '.')  }}</td>
                                            <td>
                                            @if($negocio->organizacao != null)
                                                {{ $negocio->organizacao }}
                                            @elseif($negocio->pessoas != null)
                                                {{ $negocio->pessoas }}
                                            @endif
                                            </td>
                                            <td>{{ date('d/m/Y', strtotime($negocio->data_criacao)) }}</td>
                                            <td>
                                            @if($negocio->status == 0)
                                                {{ 'EM ABERTO' }}
                                            @elseif($negocio->status == 1)
                                                {{ 'PERDIDO' }}
                                            @elseif($negocio->status == 2)
                                                {{ 'GANHO' }}
                                            @endif
                                            </td>
                                        </tr>
