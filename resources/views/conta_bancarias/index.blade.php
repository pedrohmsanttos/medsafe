...
@section('content')
    ...
    @include('conta_bancarias.filter')
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('contasbancarias/create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('conta_bancarias.table')
            <div class="row">
                <div class="col-md-6">
                <p style="margin-top: 25px;">Mostrando página {{ $contas->currentPage() }} de {{ $contas->lastPage() }} página(s), total de {{ $contas->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'contasbancarias/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'contasbancarias/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Toda(s) a(s) Conta(s) Bancária(s) inativado(s) não ficaram disponivél no sistema!",
                    type: "warning",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar as Conta(s) Bancária(s) selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Suas Conta(s) Bancária(s) não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
