...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select id="campo">
                                ...
                                <option value="lancamentopagar_id">Lançamento</option>
                            </select>
                        </div>
                        <div class="col-md-3 form-line">
                            ...
                            <div id="valorLancamento">
                                <select data-live-search="true" id="lancamentopagar">
                            ...
                            <div id="valorPagamento">
                                <select data-live-search="true" id="pagamento">
                                    <option value="">Selecione a forma de pag.</option>
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/baixapagar" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                <div class="col-md-10">
                    <h2>Lista de Baixa de Lançamento a Pagar</h2>
                </div>
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('baixapagar.create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('baixa_pagars.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $baixapagar->currentPage() }} de {{ $baixapagar->lastPage() }} página(s), total de {{ $baixapagar->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'baixapagar/'+$(this).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'baixapagar/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                //window.location = 'baixa_pagars/'+$('.checkbox:checked')[indice].value+'/edit'
                swal({
                    title: "Você tem certeza?",
                    text: "Todo(s) a(s) baixa(s) inativa(s) perderá(ão) acesso ao sistema!",
                    type: "warning",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar as baixas selecionadas", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Suas baixas não foi(ram) inativada(s) :)", "error");
                    }
                });
            }else{
        ...
        $(".add").click(function(){
            ...
            if(opFiltro == 'orderBy'){
                ...
                valor = $('#pagamento').val();
            }else if(opCampo == 'lancamentopagar_id'){
                valor = $('#lancamentopagar').val();
            }else{
                valor = $('#valor').val();
            }
        ...
        $('#campo').on('change',function() {
            ...
            if(opcao == 'situacao'){
                ...
                $('#valorSituacao').hide();
                $('#campo').selectpicker('render');
            }else if(opcao == 'lancamentopagar_id'){
                $('#valor').hide();
                $('#valorOrdem').hide();
        ...
        $('#filtro').on('change',function() {
            ...
            if(opcao == 'orderBy'){
                    ...
                    $("#campo option[value='lancamentopagar_id']").prop('disabled',true);
                    $("#campo").val('');
                    $('#campo').selectpicker('render');
                }else{
                    ...
                    $("#campo option[value='lancamentopagar_id']").prop('disabled',false);
                    $("#campo").val('');
                    $('#campo').selectpicker('render');
                }
        });
