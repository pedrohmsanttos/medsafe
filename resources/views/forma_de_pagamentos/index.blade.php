...
@section('content')
    ...
    @include('forma_de_pagamentos.filter')
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('formaDePagamentos/create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('forma_de_pagamentos.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $formaDePagamentos->currentPage() }} de {{ $formaDePagamentos->lastPage() }} página(s), total de {{ $formaDePagamentos->total() }} registros <p id="selecionados"></p></p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $formaDePagamentos->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'formaDePagamentos/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'formaDePagamentos/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                //window.location = 'clientes/'+$('.checkbox:checked')[indice].value+'/edit'
                swal({
                    title: "Você tem certeza?",
                    text: "Toda(s) a(s) forma(s) de pagamento inativa(s) perderá(ão) o acesso pelo sistema!",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "Sim, Inativá-la(s)!",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar as formas de pagamento selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Sua(s) forma(s) de pagamento não foi(ram) inativada(s)!", "error");
                    }
                });
            }else{
