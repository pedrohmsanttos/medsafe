...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select id="campo">
                                ...
                                <option value="classe_bonus">Classe bônus</option>
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/segurados" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
                                    ...
                                    @foreach($filters as $filter)
                                        <tr class="teste">
                                            ...
                                            @if(strlen($nomeCliente) > 0 )
                                                <td class="center" id="tr-init">Cliente</td>
                                                <td class="center" id="tr-init">{{ $nomeCliente }}</td>
                                            @else
    ...
    <div class="card">
        ...
        <div class="body">
            @include('segurados.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $segurados->currentPage() }} de {{ $segurados->lastPage() }} página(s), total de {{ $segurados->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'segurados/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'segurados/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os segurados selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus segurados não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
       ...
       var urlOrdem  = '';
        ...
        $('#campo').on('change',function() {
            ...
            if(opcao == 'situacao'){
                ...
                $('#valorCliente').hide();
            }else if(opcao == 'cliente_id' && opSelecionada != 'orderBy'){
                $('#valor').hide();
                $('#valorOrdem').hide();
                $('#valorSituacao').hide();
                $('#campo').selectpicker('render');
        ...
        $('#filtro').on('change',function() {
            ...
            if(opcao == 'orderBy'){
                ...
                $('#valorOrdem').show();
                $("#campo option[value='situacao']").prop('disabled',true);
                $("#campo option[value='cliente_id']").prop('disabled',true);
                $("#campo").val('');
                $('#campo').selectpicker('render');
                $('#valorCliente').hide();
            }else{
                ...
                $('#valor').show();
                $("#campo option[value='situacao']").prop('disabled',false);
                $("#campo option[value='cliente_id']").prop('disabled',false);
                $("#campo").val('');
                $('#campo').selectpicker('render');
            }
        });
