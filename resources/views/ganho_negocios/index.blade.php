...
@section('content')
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/ganhoNegocios" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                <div class="col-md-10">
                    <h2>Lista de Ganho Negocios</h2>
                </div>
                <ul class="header-dropdown ">
                     <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('ganhoNegocios.create') !!}">Adicionar novo</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('ganho_negocios.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $ganhoNegocios->currentPage() }} de {{ $ganhoNegocios->lastPage() }} página(s), total de {{ $ganhoNegocios->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'ganho_negocios/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Todo(s) o(s) Ganho(s) de Negócio(s) inativados perderá(ão) o acesso ao sistema!",
                    type: "warning",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os ganho_negocios selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus ganho_negocios não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
