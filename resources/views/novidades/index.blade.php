    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                @if (!Auth::user()->hasRole('corretor_user'))
                    <ul class="header-dropdown ">
                        <li>
                            <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('novidades/create') !!}">Adicionar</a>
                        </li>
                        <li class="dropdown">
        ...
        <div class="body">
            @include('novidades.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $novidades->currentPage() }} de {{ $novidades->lastPage() }} página(s), total de {{ $novidades->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'novidades/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'novidades/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Toda(s) a(s) Novidade(s) inativada(s) não poderá(ão) ser mais utilizadas no sistema!",
                    type: "warning",
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar as novidades selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Novidades não foi(ram) inativado(s)", "error");
                    }
                });
            }else{
