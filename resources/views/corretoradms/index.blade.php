...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select id="campo">
                                ...
                                <option value="periodo_de_pagamento">Período de Pagamento</option>
                            </select>
                        </div>
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/corretoradms" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        ...
        <div class="body">
            @include('corretoradms.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $corretoradms->currentPage() }} de {{ $corretoradms->lastPage() }} página(s), total de {{ $corretoradms->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'corretoradms/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'corretoradms/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os corretoradms selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus corretoradms não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
