...
@section('content')
    ...
    @include('motivo_perda_negocios.filter')

    <div class="card">
        <div class="header">
            <div class="row">
                <div class="col-md-10">
                    <h2>Lista de Motivos de Perda de Negócio</h2>
                </div>
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('motivoPerdaNegocios/create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
        @include('motivo_perda_negocios.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $motivoPerdaNegocios->currentPage() }} de {{ $motivoPerdaNegocios->lastPage() }} página(s), total de {{ $motivoPerdaNegocios->total() }} registros <p id="selecionados"></p> </p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $motivoPerdaNegocios->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'motivoPerdaNegocios/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'motivoPerdaNegocios/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Toda(s) a(s) Perda(s) de Negócio(s) inativo(s) perderá(ão) o acesso ao sistema!",
                    type: "warning",
                    ...
                    if (isConfirm) {
                        var motivoPerdaNegocios = [];
                        $.each($("input:checked"), function(){            
                            if($(this).val() != 'on'){
                                motivoPerdaNegocios.push($(this).val());
                            }
                        });
                        ...
                        var request = $.ajax({
                            url: 'motivoPerdaNegocios/1',
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}',
                                _method: 'DELETE',
                                ids: motivoPerdaNegocios
                            },
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os motivoPerdaNegocios selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus Motivo(s) de Perda(s) de Negocio(s) não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
