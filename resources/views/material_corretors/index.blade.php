...
@section('content')
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/materialCorretors" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        ...
        <div class="body">
            @include('material_corretors.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $materialCorretors->currentPage() }} de {{ $materialCorretors->lastPage() }} página(s), total de {{ $materialCorretors->total() }} registros <p id="selecionados"></p> </p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $materialCorretors->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'materialCorretors/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'materialCorretors/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                //window.location = 'material_corretors/'+$('.checkbox:checked')[indice].value+'/edit'
                swal({
                    ...
                    if (isConfirm) {
                        var material_corretors = [];
                        $.each($("input:checked"), function(){            
                            if($(this).val() != 'on'){
                                material_corretors.push($(this).val());
                            }
                        });
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os material_corretors selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus material_corretors não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
