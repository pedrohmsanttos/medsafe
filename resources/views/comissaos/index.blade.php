...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select id="campo">
                                ...
                                <option value="status_aprovacao">Status aprovação</option>
                            </select>
                        </div>
                        <div class="col-md-3 form-line">
                            ...
                            <div id="valorAprovacao">
                                <select data-live-search="true" id="aprovacao">
                    ...
                    <div class="row table-responsive">
                        <table class="table" id="filtros">
                            <thead>
                                ...
                                <th>
                                @if(count($filters) > 0)
                                    <a href="/comissaos" class="btn btn-primary waves-effect">Limpar</a>
                                @endif
                                </th>
                            </thead>
                            <tbody>
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! route('comissaos.create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('comissaos.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $comissaos->currentPage() }} de {{ $comissaos->lastPage() }} página(s), total de {{ $comissaos->total() }} registros <p id="selecionados"></p> </p>
                </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'comissaos/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'comissaos/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os comissaos selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Seus comissaos não foi(ram) inativado(s) :)", "error");
                    }
                });
            }else{
       ...
       var urlOrdem  = '';
        ...
        $(".add").click(function(){
            ...
            if(opFiltro == 'orderBy'){
                ...
                valor = $('#status').val();
                situacao += valor;
            }else if(opCampo == 'status_aprovacao'){
                valor = $('#aprovacao').val();
            } else{
                valor = $('#valor').val();
            }
        ...
        $('#campo').on('change',function() {
            ...
            if(opcao == 'situacao'){
                ...
                $('#valorAprovacao').hide();
            }else if(opcao == 'status_aprovacao' && opSelecionada != 'orderBy'){
                $('#valor').hide();
                $('#valorOrdem').hide();
                $('#valorSituacao').hide();
                $('#campo').selectpicker('render');
        ...
        $('#filtro').on('change',function() {
            ...
            if(opcao == 'orderBy'){
                ...
                $('#valorOrdem').show();
                $("#campo option[value='situacao']").prop('disabled',true);
                $("#campo option[value='status_aprovacao']").prop('disabled',true);
                $("#campo").val('');
                $('#campo').selectpicker('render');
                $('#valorAprovacao').hide();
            }else{
                ...
                $('#valor').show();
                $("#campo option[value='situacao']").prop('disabled',false);
                $("#campo option[value='status_aprovacao']").prop('disabled',false);
                $("#campo").val('');
                $('#campo').selectpicker('render');
            }
        });
