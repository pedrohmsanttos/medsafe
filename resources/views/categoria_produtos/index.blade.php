...
@section('content')
    ...
    @include('categoria_produtos.filter')
    ...
    <div class="card">
        <div class="header">
            <div class="row">
                ...
                <ul class="header-dropdown ">
                    <li>
                        <a class="btn btn-success waves-effect pull-right" style="margin-top:-10px;margin-right: 10px;" href="{!! url('categoriaProdutos/create') !!}">Adicionar</a>
                    </li>
                    <li class="dropdown">
        ...
        <div class="body">
            @include('categoria_produtos.table')
            <div class="row">
                <div class="col-md-6">
                    <p style="margin-top: 25px;">Mostrando página {{ $categoriaProdutos->currentPage() }} de {{ $categoriaProdutos->lastPage() }} página(s), total de {{ $categoriaProdutos->total() }} registros <p id="selecionados"></p></p>
                </div>
                <div class="col-md-6 rigth">
                    {!! $categoriaProdutos->links() !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(".gridBody td").click(function(){
            if(!$(this).hasClass('checkItem')){
                ...
                window.location = 'categoriaProdutos/'+$( this ).parent().data( "id" )+'/edit'
            }
        });
        ...
        $('#editar').on('click', function(){
            ...
            if($('.checkbox:checked').length < 2 && $('.checkbox:checked').length > 0){
                window.location = 'categoriaProdutos/'+$('.checkbox:checked')[indice].value+'/edit'
            }else{
        ...
        $('#deletar').on('click', function(){
            ...
            if($('.checkbox:checked').length > 0){
                ...
                swal({
                    ...
                    text: "Toda(s) a(s) categoria(s) de produto inativa(s) perderá(ão) o acesso ao sistema!",
                    type: "warning",
                    ...
                    if (isConfirm) {
                        var categoria_produtos = [];
                        $.each($("input:checked"), function(){            
                            if($(this).val() != 'on'){
                                categoria_produtos.push($(this).val());
                            }
                        });
                        ...
                        var request = $.ajax({
                            ...
                            success: function(result){
                                ...
                                if(result.msg == 'Sucesso'){
                                    ...
                                    swal("Erro", "Não foi possivel inativar os categorias de produtos selecionados", "error");
                                }
                            }
                        });

                    } else {
                        swal("Cancelado", "Categoria de produtos não foram inativadas", "error");
                    }
                });
            }else{
