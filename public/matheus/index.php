...
//sysbench /usr/share/sysbench/oltp_read_write.lua --mysql-host=192.168.1.9 --mysql-port=3306 --mysql-user=root --mysql-password='!nhaltdados10000' --mysql-db=benchmark_matheus --db-driver=mysql --tables=3 --table-size=10000000  prepare
$options['db.host'] = '192.168.1.9';
$options['db.user'] = 'root';
$options['db.pw']   = '!nhaltdados10000';
$options['db.name'] = 'benchmark_matheus';
...
function test_mysql(&$result, $settings)
{
    ...
    $link = new PDO('mysql:host='.$settings['db.host'].';dbname='.$settings['db.name'], $settings['db.user'], $settings['db.pw']);
    $result['benchmark']['mysql_connect'] = timer_diff($timeStart) . ' sec.';
    // nda
    $result['benchmark']['mysql_select_db'] = timer_diff($timeStart) . ' sec.';

    $dbResult = $link->query('SELECT VERSION() as version;', PDO::FETCH_ASSOC)->fetch();
    $result['sysinfo']['mysql_version'] = $dbResult['version'];
    $result['benchmark']['mysql_query_version'] = timer_diff($timeStart) . ' sec.';

    $dbBenchmark = $link->query("SELECT BENCHMARK(1000000, AES_ENCRYPT('hello', UNHEX('F3229A0B371ED2D9441B830D21A390C3')));", PDO::FETCH_ASSOC)->fetch();
